...
The \emph{Europen Convetion on Human Rights} \cite{ECHR} was created by the members of the Europen Council in 1953 as part of the aftermath of the second world war.
...
EU directives in general do not hold any direct legal binding for EU citizens, but has to be implemented by national law of the EU memberstates.

It roughly follows the OECD guidelines for privacy protection issues in 1980 \cite{OECD80}.

The directive regulates the processing of personal data regardless of whether such processing is automated or not.
Where \emph{personal data} is any information relating to an identified or identifiable natural person [...] (Art. 2a)
and \emph{processing} is any operation or set of operations which is performed upon personal data [...] (Art. 2b).

The identified or identifiable natural person is called \emph{data subject}.

The sole responsibility for compliance is held by so called \emph{controllers}.
...
This rule is applicable whenever the controller uses equipment situated within the EU.
This will hold for every e-commerce provider inside or outside of the EU because the customer's computer is situated within the EU anyways.
...
\begin{itemize}
\item \textbf{Transparency}  (\cite{1} art. 10,11): ``The data subject has the right to be informed when his personal data is being processed [...]'' and he has the right how and by whom it is processed.
Additionally other specified requirements have to be met, i.e.~giving consent.
\item \textbf{Legitimate purpose} (\cite{1} art. 6b): ``Personal data can only be processed for specified explicit and legitimate purposes and may not be processes further in a way incompatible with those purposes ''.
\item \textbf{Proportionality}: ``Personal data may be processed only insofar as it is adequate [...] in relation to [its] purposes [...]; every reasonable step must be taken to ensure that data which are inaccurate or incomplete [...] are erased or rectified (art. 6)''
\end{itemize}
