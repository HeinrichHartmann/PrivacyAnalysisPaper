...
They felt a need for a new right to "protect the extent to wich one's thoughts, sentiments, and emotions could be shared with others" \cite{1}.
This right would be "the right to be left alone".

The right to "protect the extent to wich one's thoughts, sentiments, and emotions could be shared with others" by Warren and Brandeis was later extend to cover arbitrary information and is now kown as the privacy concept "... the control we have over information about ourselves ..." \cite{2}.
...
interpretation in its juresprudence.'' \cite{2}
...
\cite{1}.
\item
\textbf{Proportionality}: ``Personal data may be processed only insofar as it is adequate [...] in relation to [its] pruposes [...]; every reasonable step must be taken to ensure that data wich are inaccurate or incomplete [...] are erased or rectified (art. 6)'' \cite{1}
\end{itemize}
...
Federal public authorites are only bound to their federal law. \cite{2}
...
The act is knwon for its high complexity: a manual record of phone numbers for business purposes could be hold subject to the DPA \cite{3}.
...
France implements Directive 95/46 with Law 2004-801 modifying law 78-17 of 6.1.1978 (\emph{Loi n° 2004-801 du 6 août 2004} modifying \emph{loi n°78-17 relative à l'informatique, aux fichiers et aux libertés}) \cite{1}\cite{10}\cite{11}.
...
Many privacy researchers seem to ``forcus on the ways in which privacy can be infrenged'' \cite{1}.
...
This type is concerned with ``making sure that individuals's data is not automatically available to other individuals and organisations''. This is Informational Privacy in an intuitive sense.
...
This type is the right ``to move about in public or semi-public space withoug being identified, tracked or monitored''.
...
This type is the right ``to associate with whomever [one] wishes, withoug being monitored''.
...
\end{itemiz*iPrivacy of Behaviour and Action}
he Privacy of Behaviour and Action is concerned with one' and social activities (religious, po...l, sexual, \ldots). Ifhis type is violated, following implicit violations are possible:
item
  \textbf{Privacy of The Person:} Religious practices emph include body
...
observation. For the Live+Gove project this type is related to t is recirded by default and by linking it to maps a and li phone books those *``personal matters*" could be
disclosed by frequently visited places (c...s, brothels, party headuaters, \emph).

communications' either
electronic or face-to-face''} \cite{1}. This type
