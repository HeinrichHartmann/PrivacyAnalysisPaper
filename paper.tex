...
\textbf{Privacy Dashboard (R1)}. In order to address the threat with the highers risk, Insufficient Control (T1) of the citizen, we need to give the citizen back the
...
\textbf{Privacy Policy (R2)}. A necessary pre-requirement for effective control of the citizen over his data is information and comprehension of the intended data capturing and processing steps. Therefore a policy that
is easily readable and contains all important information is essential and a legal requirement.

\textbf{Data Handling Guidelines (R3)} The threat with the second largest risk is (T2) Excessive Data Mining.
...
\textbf{IT Infrastructure Security (R6)}. The protection from threat scenario (T3) Data Theft is a case of
...
\textbf{Encryption (R7)} The protection from threat scenario (T4) Surveillance focuses on the
...
\textbf{Relationship (R8)}. Threat scenarios (T5) Information Leak and (T6) Social Engineering
